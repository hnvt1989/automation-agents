...
-- Version with temporary memory increase for index creation
...
-- Create GIN index for full-text search (with CONCURRENTLY to avoid blocking)
DROP INDEX IF EXISTS idx_document_embeddings_content_search;
CREATE INDEX CONCURRENTLY idx_document_embeddings_content_search 
ON document_embeddings USING GIN(content_search_vector);

-- Create trigram index for fuzzy matching (with CONCURRENTLY to avoid blocking)
DROP INDEX IF EXISTS idx_document_embeddings_content_trgm;
CREATE INDEX CONCURRENTLY idx_document_embeddings_content_trgm 
...
DROP INDEX IF EXISTS idx_document_embeddings_has_context;
CREATE INDEX CONCURRENTLY idx_document_embeddings_has_context 
ON document_embeddings(has_context) 
WHERE has_context = TRUE;

-- 6. Create composite index for collection queries
DROP INDEX IF EXISTS idx_document_embeddings_collection_embedding;
-- Note: We skip this index as it would be very large with embeddings

-- 7. Add support for metadata search
DROP INDEX IF EXISTS idx_document_embeddings_metadata;
CREATE INDEX CONCURRENTLY idx_document_embeddings_metadata 
